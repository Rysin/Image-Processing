...
def pathValidation():
	...
	if os.path.isfile(user_path):
		...
		print('Invalid Path! Please provide appropriate path.')
		print('Exiting for now! Try Again ☺')
		sleep(1.5)
		sys.exit()
...
def findChroma(videofile):
	...
	# plt.show()
	print('Green can found at timestamps given below, if any\n')
...
logging.basicConfig(filename="Results.log", format='%(asctime)s %(message)s', filemode='w')
logger = logging.getLogger()
logger.setLevel(logging.INFO)

flag, path = pathValidation()
timeStamps = []

if flag:
	files = os.listdir(path)
	for file in files:
		if str(file).endswith('.wmv') or str(file).endswith('.mp4'):
			f_path = os.path.join(path, file)
			print(f_path)
			T_dir = findChroma(f_path)
			logger.info(str(f_path))
			if len(T_dir) != 0:
				logger.info(str(T_dir))
				logger.info('\n')
			else:
				logger.info('No Green Content found in this file.')
				logger.info('\n')
			
			continue
		else:
			print('Input File Is Not Of Supported Video Format! Hence Moving On!')
			continue
else:
	T_file = findChroma(path)
	logger.info(path)
	if len(T_file) != 0:
		logger.info(str(T_file))
	else:
		logger.info('No Green Content found in this file.')
		logger.info('\n')
